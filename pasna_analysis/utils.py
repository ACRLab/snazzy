...
def mean_ci(data, ci=95, axis=0, bootstrap=True, n_boot=10000):
    ...
    if bootstrap:
        boots = sns.algorithms.bootstrap(
            a, n_boot=1000, func=np.nanmean, axis=axis)
...
def generate_standard_timestamp(timestamps, trim=False, min_time=None, max_time=None):
    ...
    if trim:
        min_time = np.nanmax(np.nanmin(timestamps, axis=1)
                             ) if min_time is None else min_time
        max_time = np.nanmin(np.nanmax(timestamps, axis=1)
                             ) if max_time is None else max_time
    else:
...
def align_traces_to_standardized_timestamp(ts, xs, ts_standard=None, trim=False, min_time=None, max_time=None):
    ...
    xs_standardized = np.array([interpolate_to_new_timestamp(
        xs[i], ts[i], ts_standard) for i in range(len(xs))])
