...
class PasnaFly():
    ...
    def __init__(self, csv_path, remove_artifacts=True, artifact_threshold=300, trim_data=True, trim_zscore=0.35):
        ...
        self.df_raw = self.import_data(
            remove_artifacts=remove_artifacts, artifact_threshold=artifact_threshold)
        ...
        if trim_data:
            ...
            print(len(self.df_raw_untrimmed))
            self.df_raw, self.trim_idx = self.trim_data(
                trim_zscore=trim_zscore)
    ...
    def import_data(self, remove_artifacts=True, artifact_threshold=1500):
        ...
        try:
            ...
            df = pd.read_csv(self.csv_path, encoding='utf-8', header=[1])
        df.columns = ['time', 'gcamp', 'tomato']
        if remove_artifacts:
            arts_gc = np.logical_or(
                df.gcamp > artifact_threshold, df.gcamp == 0)
            df.gcamp.loc[arts_gc] = np.nan
            df.gcamp.loc[arts_gc] = np.interp(
                df.time[arts_gc], df.time[~arts_gc], df.gcamp.loc[~arts_gc])
            arts_td = np.logical_or(
                df.tomato > artifact_threshold, df.tomato == 0)
            df.tomato.loc[arts_td] = np.nan
            df.tomato.loc[arts_td] = np.interp(
                df.time[arts_td], df.time[~arts_td], df.tomato.loc[~arts_td])
    ...
    def trim_data(self, trim_zscore=5):
        ...
        tomato_savgol = spsig.savgol_filter(
            self.df_raw.tomato, 251, 2, deriv=0)
    ...
    def compute_ratiom_gcamp(self, remove_artifacts=False):
        ...
        if remove_artifacts:
            arts = ratiom > 15
            ratiom[arts] = np.nan
            ratiom[arts] = np.interp(
                self.time[arts], self.time[~arts], ratiom[~arts])
        return ratiom
    ...
    def compute_peak_bounds(self, rel_height=0.92):
        ...
        peak_widths_idxes, _, peak_left_idxes, peak_rights_idxes = spsig.peak_widths(
            self.savgol, self.peak_idxes, rel_height)
        ...
        self.peak_bounds_exact_idxes = np.dstack(
    ...
    def get_peak_slices_from_bounds(self, left_pad=0, right_pad=0):
        ...
        '''
        peak_left_idxes_rnd = np.round(
            self.peak_bounds_exact_idxes[:, 0]).astype(int)
        peak_right_idxes_rnd = np.round(
            self.peak_bounds_exact_idxes[:, 1]).astype(int)
        peak_edges_idxes_rnd = np.dstack(
            (peak_left_idxes_rnd-left_pad, peak_right_idxes_rnd+right_pad)).squeeze()
    ...
    def get_peak_slices_from_peaks(self, left_pad=3, right_pad=10):
        ...
        '''
        peak_edges_idxes_rnd = np.dstack(
            (self.peak_idxes-left_pad, self.peak_idxes+right_pad)).squeeze().reshape(-1, 2)
        peak_slices = [self.savgol[x[0]:x[1]] for x in peak_edges_idxes_rnd]
        time_slices = [self.time[x[0]:x[1]] for x in peak_edges_idxes_rnd]
        return list(zip(peak_slices, time_slices))
    ...
    def compute_peak_aucs_from_bounds(self, left_pad=0, right_pad=0):
        ...
        peak_aucs = np.asarray([np.trapz(pslice*100, tslice)
                               for pslice, tslice in peak_time_slices])  # %*min
    ...
    def compute_peak_aucs_from_peaks(self, left_pad=3, right_pad=10):
        ...
        Assumes that detect_peaks has been called.
        '''
        peak_time_slices = self.get_peak_slices_from_peaks(
            left_pad=left_pad, right_pad=right_pad)
        peak_aucs = np.asarray([np.trapz(pslice*100, tslice)
                               for pslice, tslice in peak_time_slices])  # %*min
    ...
    def plot(self, raw=True, figsize=None):
        ...
        if raw:
            ...
            if hasattr(self, 'df_raw_untrimmed'):
                ax[0].plot(self.df_raw_untrimmed.time,
                           self.df_raw_untrimmed.gcamp, color='green', label='GCaMP')
                ax[1].plot(self.df_raw_untrimmed.time,
                           self.df_raw_untrimmed.tomato, color='#b00000', label='tdTomato')
                ...
                ax[0].plot(self.df_raw.time, self.df_raw.gcamp,
                           color='green', label='GCaMP')
                ax[1].plot(self.df_raw.time, self.df_raw.tomato,
                           color='#b00000', label='tdTomato')
            ax[0].legend()
            ax[1].legend()
            ax_processed = ax[2]
...
def compute_baseline(signal, window_size=140, n_bins=20):
    ...
    model.fit(beginning_x.reshape(-1, 1),
              signal[0:first_center].reshape(-1, 1))
    # fitted values
