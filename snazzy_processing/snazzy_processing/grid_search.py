...
from snazzy_processing import centerline_errors, vnc_length
...
def search(
    x_list, y_list, embryos, annotated, hatching_points=None, interval=20, num_samples=5
):
    ...
    for emb in embryos[:num_samples]:
        ...
        for x in x_list:
            for y in y_list:
                ...
                error = centerline_errors.compare(lengths, annotated[emb.stem])
                errors[
                    (
                        x,
                        y,
                    )
                ].append([emb.stem, error])
    return errors
...
def get_emb_files(exp_dir, num_samples):
    ...
    annotated_files = centerline_errors.get_random_files(annotated_dir, n=num_samples)
