...
class PasnaFly:
    ...
    def detect_peaks(
        self, mpd=71, order0_min=0.06, order1_min=0.006, extend_true_filters_by=30
    ):
        """
    ...
    def compute_peak_bounds(self, rel_height=0.92):
        ...
        self.peak_bounds_exact_idxes = np.dstack(
            (peak_left_idxes, peak_rights_idxes)
        ).squeeze()
    ...
    def get_peak_slices_from_bounds(self, left_pad=0, right_pad=0):
        ...
        peak_edges_idxes_rnd = np.dstack(
            (peak_left_idxes_rnd - left_pad, peak_right_idxes_rnd + right_pad)
        ).squeeze()
    ...
    def get_peak_slices_from_peaks(self, left_pad=3, right_pad=10):
        ...
        """
        peak_edges_idxes_rnd = (
            np.dstack((self.peak_idxes - left_pad, self.peak_idxes + right_pad))
            .squeeze()
            .reshape(-1, 2)
        )
        peak_slices = [self.savgol[x[0] : x[1]] for x in peak_edges_idxes_rnd]
    ...
    def plot(self, raw=True, figsize=None):
        ...
        if raw:
            ...
            if hasattr(self, "df_raw_untrimmed"):
                ax[0].plot(
                    self.df_raw_untrimmed.time,
                    self.df_raw_untrimmed.gcamp,
                    color="green",
                    label="GCaMP",
                )
                ax[1].plot(
                    self.df_raw_untrimmed.time,
                    self.df_raw_untrimmed.tomato,
                    color="#b00000",
                    label="tdTomato",
                )
                ...
                ax[0].plot(
                    self.df_raw.time, self.df_raw.gcamp, color="green", label="GCaMP"
                )
                ax[1].plot(
                    self.df_raw.time,
                    self.df_raw.tomato,
